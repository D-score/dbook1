...
Moreover, we assume that the development can be measured by one construct,
the $D$-score. We can evaluate to what extend the fit of our model 
...
mutate(lex_gsed = gseddata::rename_gcdg_gsed(item),
       domain = gseddata::decompose_itemnames(lex_gsed)[, "domain"],
       domain = plyr::revalue(domain, c("cm" = "Communication", 
                                        "fm" = "Fine motor",
                                        "gm" = "Gross motor")),
      lex_gcdg = item) %>%
  left_join(model$itembank[,c("lex_gcdg", "tau")], by = "lex_gcdg") %>%
...
Figure \@ref(fig:plotdomainfit) shows that the item fit shows no domain
clustering. This means that the item fit residuals of the model cannot be
explained by differences between the DDI domains. 
...
D-score can discriminate respondents that are not developing normally
...
correlation to other construct should be at least 0.6. Unfortunately,
only limited data are available for the DDI, so convergent validity 
...
```{r plotconcurrent, eval=TRUE, fig.height=3, fig.show='hold', message=FALSE, warning=FALSE}
