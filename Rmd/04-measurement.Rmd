...
illustrates the process using Dutch data. 
...
months. The YHC professionals administer an age-appropriate subset of
...
(SMOCC) study is a nationally representative cohort of 2,151
...
The item are colored by domain (blue: gross motor, green: fine motor,
red: communication). In general, domains are well mixed across age,
though around some ages, e.g., at 4 months, multiple milestones from
the same domain appear.
...
DDI against the $D$-score (Source: SMOCC data, 2151 children, 9 occasions).
...
Figure \@ref(fig:smoccda) shows that the relation between $D$-score
and age is nonlinear. Development in the first year is more rapid than
...
the vertical axis), we see a curvelinear relation between the age and
item difficulty.
...
\@ref(fig:smoccpa) (with *unequal slopes*). Curves are colored by
domain.
...
All patterns can co-exist because of the curvature in the relation
between $D$-score and age. The curvature is never explicitly modeled
or defined, but a consequence of the equal-slopes assumption in the
relation between the $D$-score and the passing percentage of a
milestone.
...
*logistic units* (or  *logits*) [@berkson1944]. The reason for using
the logistic rather than linear units is that its output returns a
probability value that is mapped to discrete events. In our case, we
can describe the probability of passing an item (milestone) as a
function of the difference between $\beta_n$ and $\delta_i$ expressed
in logits.
...
$\delta_i$, so we may use the graph in two ways:
...
difficulty of the item is equal to the child's ability, then child has
...
located on a *single* underlying latent variable. This means that the
relations between the items are fully explainable by the scores on the
latent scale. In practice, the requirement implies that items should
measure the same construct. (@hattie1985)
...
in Chapter \@ref(ch:evaluation), the Rasch model provides excellent fit to
