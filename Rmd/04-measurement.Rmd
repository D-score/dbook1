...
```{r smoccpa, fig.cap = '(ref:smoccpa)', fig.width = 7, fig.height = 4}
...
# suppress the group_by_() deprecation warning
py <- suppressWarnings(plotly::ggplotly(plot, tooltip = c("item", "label")))
py
```
...
```{r smoccpd, fig.cap = '(ref:smoccpd)', fig.width = 7, fig.height = 4}
modelb <- model$dscore
...
py <- plotly::plot_ly(tab, x = ~a, y = ~d, z = ~p, color = ~domain, 
...
Let us now introduce a few symbols. We adopt the notation used in @wright1982. See Appendix \@ref(ap:notation) for a complete list.
...
-   *Fits child development data*: Last but not least, as we will see in Chapter \@ref(ch:evaluation), the Rasch model provides an excellent fit to child development milestones.
