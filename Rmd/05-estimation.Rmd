...
p1m <- data.frame(qp = qp, month = rep(1, length(qp)), p = adp_dutch(1/12, qp))
p15m <- data.frame(qp = qp, month = rep(15, length(qp)), p = adp_dutch(15/12, qp))
p24m <- data.frame(qp = qp, month = rep(24, length(qp)), p = adp_dutch(24/12, qp))
...
david[, "prior1"] <- adp_dutch(t = 15/12)
david[, "prior1"] <- post[1, ]
...
post <- dscore_posterior(data = data_rob, qp = -10:80, prior_mean = ".dutch")

rob[, "prior1"] <- adp_dutch(t = 15/12)
rob[, "prior1"] <- post[1, ]
