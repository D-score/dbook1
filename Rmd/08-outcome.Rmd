...
In the first step, the score of the first item is combined with the prior. However, at the first step we cannot use the previous posterior as our prior. Hence, we need a starting distribution to use as the prior in the first step. 

As start distribution it was decided to choose a quite broad distribution which was centred quite well. 
...
```{r, posterior 1e item, echo=FALSE}
scores1 <- tableWS1
items <- c("n34")
age <- 1.25
...
```{r, posterior 2e item PP, echo=FALSE}
items <- c("n34", "n35")
...
```{r, posterior 2e item PF, echo=FALSE}
items <- c("n34", "n35")
age <- rep(1.25, length(items))
...
```{r, posterior both children and prior, echo=FALSE}
