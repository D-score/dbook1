...
the probability of passing each item at a given level of ability.

The figure shows that the probability of passing an item increases
with ability. Items are ordered in difficulty from left to right. Item
`stepping` is the easiest and `walk_alone` is most difficult. The
right. The point at which a logistic curve crosses the 50 percent line
(marked by a cross) is customarily taken as the *difficulty of the item*.
...
1. An important assumption is that the items "measure the same thing",
or put differently, are manifestations of an underlying continuous
latent variable. Unit-based measurement won't work if there is no
...
------------------------- | --------- | ----------- | ----------
Behavior as outcome       |   NO      | YES         | YES
Independent of age norm   |   NO      | YES         | YES
Supports multiple items   |   NO      | YES         | YES
Latent variable           |   NO      | YES         | YES
Robust to item skipping   |  YES      | NO          | YES
Comparable scores         |  YES      | NO          | YES
Independent of instrument |  YES      | NO          | YES
...
Age-based measurement does not support multiple items, and does not
used the concept of a latent variable.

*Score-based measurement* quantifies development by summing the number
of passes. Different instruments make different selections of items,
...
Corrections are needed if items are skipped or added.
