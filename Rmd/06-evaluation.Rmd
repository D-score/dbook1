...
probabilities at various levels of ability. Figure \@ref(fig:ploticc)
shows the item characteristics curves of two DDI milestones. The
...
according to the Rasch model. The empirical and estimated curves are
...
three hypothetical items. These items were simulated to have a low fit,
added to the DDI, and their parameters were estimated by the methods
...
this item essentially only adds to the noise. Item `hypgmd002`
converges to an asymptote around 80 percent and has a severe dip in
the middle. The dip is caused by the strong relation to age. Item
`hypgmd003` appears to have the wrong coding. In addition, the
spike-like behavior in the middle is often seen when two or more
different items are erroneously given identical item names.
...
Figure \@ref(fig:plothyp2) shows two hypothetical items with two forms
of overfit. The curve of item `hypgmd004` is much steeper than the
modeled curve. Thus, just this one item is exceptionally well-suited
to distinguish children with a $D$-score below 50$D$ from those with a
...
should make certain though, that FAIL and PASS scores are all measured
(not imputed) values.
...
person $n$ on item $i$ can be $0$ or $1$. 
...
child conform to the Rasch model expectation. The Rasch model expects
that a more able child has a greater probability to pass an item than
a less developed child. Person fit analysis evaluates the extent to
which this is true.
...
Outfit is high, for example, when the child fails easy items but
...
As a threshold for person fit < 3.0 can be used to clean out children
...
all milestones in the model) is fair for both boys and girls, we see
...
intersection of 50 percent). Conversely, milestone `ddigmm064`
...
The DDI did not contain items for which the ability-group interaction
was statistically significant, so we conclude that there is no
...
considerably between items. Using 0.05 as a criterion, milestone
`ddigmm060` is sensitive between ages 4--8 months, whereas `ddicmm039`
is sensitive between ages 10--19 months, a range that is about twice
as broad. The symmetric nature of the curves in Figure \@ref(fig:iic)
is not present Figure \@ref(fig:iia). In general, the relation between
age and item sensitivity is more complex than the relation between
ability and item sensitivity.

The item information by age curve helps to determine at what ages the
item can best be administered. The item will be most informative if
administered at the age at which 50% of the children will pass the
milestone, which is the point where the item information is equal to
0.5 * 0.5 = 0.25. Administering the item closely around that age
provide the most efficient measurement of ability. When space is at
premium (e.g. as in population surveys) using a well-chosen set of
age-sensitive milestones will help in reducing the total number of
milestones that needs to be measured.
...
In general, a high reliability is desirable, so the reliability of a
test is often used to decide between instruments. Cronbach's $\alpha$
is a popular estimate of the lower bound of the reliability of a test.
In the Rasch model, we can estimate reliability by the
[ratio](http://www.rummlab.com.au/rmrelidx2030)
...
way as Cronbach's $\alpha$, for which typically any value beyond 0.9
is classified as *excellent*. Note that the reliability is very high
