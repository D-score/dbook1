...
```{r iic, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 4, fig.cap = '(ref:iic)'}
...
```{r iia, echo=FALSE, message=FALSE,  results = 'hide', fig.keep = 'all', warning=FALSE,  fig.height = 4, , fig.cap = '(ref:iia)'}
# model in logit
...
model_psi <- function(model, data){
  ...
  b_est <- ability(data = sim_data, items = names(model$fit$b), 
...
```{r histres, eval = FALSE, echo = FALSE}
...
measurement (sem)* is $\hat\sigma_{\hat e} = 1.32$.
