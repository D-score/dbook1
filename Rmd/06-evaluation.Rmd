...
@zumbo1999 provised a clear definition of DIF:
...
```{r plotdif1, echo = FALSE, results = 'hide', fig.keep = 'all', fig.height = 7, warning = FALSE, fig.cap = '(ref:plotdif1)', message = FALSE}
theme_set(theme_light())
p1 <- plot_dif(data = data, model = model, dif = "sex", metric = "logit", item = "ddigmd063")[[1]]
p2 <- plot_dif(data = data, model = model, dif = "sex", metric = "logit", item = "ddifmd011")[[1]]
gridExtra::grid.arrange(p1, p2)
```
...
girs, so we see no evidence of DIF here.

```{r plotdif2, echo = FALSE, results = 'hide', fig.keep = 'all', fig.height = 7, warning = FALSE, fig.cap = '(ref:plotdif2)', message = FALSE}
theme_set(theme_light())
p1 <- plot_dif(data = data, model = model, dif = "sex", metric = "logit", item = "ddicmm039")[[1]]
p2 <- plot_dif(data = data, model = model, dif = "sex", metric = "logit", item = "ddigmm067")[[1]]
gridExtra::grid.arrange(p1, p2)
```
...
boys and girls. Milestone `ddicmm039` ("Says three words") is easier
for girls by about 0.68 logits (= the difference in ability at the
intersection of 50 percent). Conversely, milestone `ddigmm067` ("Walks
while holding onto play-pen or furniture") is easier for boys by about
0.54 logits. These are the largest differences found for sex in the
...
```{r iic, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 3, fig.cap = '(ref:iic)'}
...
The item information can also be expressed against age. By doing so,
one can identify at what age an item provides the most information.
...
The item information at a given age, can ultimately help determine at
what age the item can best be administered. The item will be most
informative, when adiminered at the age at which 50% of the
respondents will pass the item. Administering the item at this age can
be helpful when the item is used in an instrument to determine the
ability of a population. However, when the item is used in a screening
instrument, the goal would be to identify respondents that
underperform compared to a norm group. In that case, the item can best
be asessed at the age where 90% of respondents will pass the item.
